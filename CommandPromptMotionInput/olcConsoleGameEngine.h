	...
	void GameThread()
	{
		...
		while (m_bAtomActive)
		{
			...
			while (m_bAtomActive)
			{
				...
				if (fElapsedTime < targetFrameTime)
				{
					std::this_thread::sleep_for(std::chrono::duration<float>(targetFrameTime - fElapsedTime));
					tp2 = std::chrono::system_clock::now(); // measure again after sleep
					elapsedTime = tp2 - tp1;
